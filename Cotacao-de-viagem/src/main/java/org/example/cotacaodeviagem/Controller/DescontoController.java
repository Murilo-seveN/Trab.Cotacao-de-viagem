...
 
    @GetMapping("/{id}") 
    public ResponseEntity<DescontoResponseDTO> buscar(@PathVariable Long id) { 
...
        return ResponseEntity.ok(service.listarPorCotacao(cotacaoId)); 
    } 
 
    @PutMapping("/{id}") 
    public ResponseEntity<DescontoResponseDTO> atualizar( 
            @PathVariable Long id, @RequestBody DescontoUpdateDTO dto) { 
